...
export default function JobExperience() {
  ...
  const jobsList = Array.isArray(experience) ? experience : [];
  const activeExperience = jobsList[activeTab] || jobsList[0] || null;
  ...
  const tabs = () =>
    jobsList.map((item, index) => (
      <li
        className={
          activeTab === index
            ? `${styles.tab} px-5 mt-1 py-3 rounded-md text-primary font-semibold`
            : "px-5 mt-1 py-3 rounded-md cursor-pointer text-secondary"
        }
        key={index}
  ...
  const tabItens = () => {
    if (!activeExperience) {
      return <p className="text-secondary">No experience data available.</p>;
    }
      ...
      <>
        <h3 className="md:text-xl text-lg font-semibold mb-1">
          ...
          <span className="ml-2 text-tertiary">
            @{activeExperience.role || activeExperience.title || "Company"}
          </span>
        </h3>
    ...
    <section id="experience" className="mt-64 relative">
      ...
      <div className="mt-20 flex items-center">
        <div className="flex items-center title">
          <Experience />
          <h3 className="min-w-max ml-4 md:text-2xl text-lg font-semibold">
      ...
      <div
        className="md:mt-20 mt-10 mx-auto md:max-w-4xl max-w-max md:px-4 px-0 sm:px-6 lg:px-8 relative"
        ref={jobs}
      >
